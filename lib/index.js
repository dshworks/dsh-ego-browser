...
export function apply(ctx, config) {
  ...
  // The tools service arrives through inject rather than as a hard dependency,
  // so a profile without it still loads. The failure that hides, though, is the
  // one where it never arrives: nothing registers, nothing errors, and the agent
  // simply has no ego tools. dsh's plugin logger does not reach the web app's
  // stdout, so a log line would not surface it either. The memory route reports
  // the registered names instead, which makes `curl .../memory` the answer to
  // "is this plugin actually working".
  let registered = []
  ctx.inject(['tools'], (toolCtx) => {
    registered = registerTools(toolCtx, {
      ego,
      memory,
      config,
